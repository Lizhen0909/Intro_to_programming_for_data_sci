...
# Use sapply to check if each number is even or odd
...
Now, solve this without using a `for` loop.
...
Vectorized operations in R are significantly faster and more efficient than loops. This is because vectorized functions are implemented in optimized C code under the hood, allowing R to apply operations to entire vectors at once instead of iterating through elements one by one.
