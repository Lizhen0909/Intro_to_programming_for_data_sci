...
If an built-in function is not available for the computations to be performed, we can write our own user-defined function within the `apply()` function
...
top_10_countries <- country_names[filtered_sorted[1:10, 1]]
...
Solve this task without using a `for` loop. Use the `lapply()` function.
